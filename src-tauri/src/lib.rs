...
fn read_sidecar(asset_path: String, relative: String) -> Result<Vec<u8>, String> {
    if !asset_path.to_ascii_lowercase().ends_with(".gltf") {
        return Err("Sidecars require a GLTF document.".into());
    }
    let parent = Path::new(&asset_path)
        .parent()
    ...
    let path = parent
        ...
        .canonicalize()
        .map_err(|e| format!("Missing resource {relative}: {e}"))?;
    if !path.starts_with(&parent) {
        return Err("Resource is outside the asset folder.".into());
    }
...
fn export_files(directory: String, name: String, files: Vec<ExportFile>) -> Result<String, String> {
    ...
    for index in 0..10000 {
        ...
        match fs::create_dir(&candidate) {
            ...
            Err(e) => return Err(format!("Cannot create output directory: {e}")),
        }
    }
    let output = selected.ok_or("Too many exports with the same name.")?;
    let result = (|| -> Result<(), String> {
        for file in files {
            let path = output.join(safe_relative(&file.path)?);
            if let Some(parent) = path.parent() {
                fs::create_dir_all(parent).map_err(|e| e.to_string())?;
            }
            fs::write(path, file.data).map_err(|e| format!("Cannot write export: {e}"))?;
        }
        Ok(())
    })();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("Error launching SpriteForge");
}
#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn reject_traversal() {
        for p in ["../evil", "/etc/passwd", "C:\\file", "a/../../file", ""] {
            assert!(safe_relative(p).is_err());
        }
        assert!(safe_relative("frames/item.png").is_ok());
    }
    #[test]
    fn export_never_overwrites() {
        ...
        .unwrap();
        let b = export_files(root.to_string_lossy().into(), "Asset".into(), vec![]).unwrap();
        assert_ne!(a, b);
        assert_eq!(fs::read(Path::new(&a).join("a.txt")).unwrap(), vec![42]);
