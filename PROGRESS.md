...
- One-click milestone: complete selected-state builds use a union of deterministic sampled bounds, shared ground anchor, existing art styles/renderer/atlas writer, cancellation and atlas-only retained output. Added playable canvas arena using actual baked atlases with movement/run/one-shots, camera-relative directions, debug, pause/slow/frame step.
- Browser environment repaired without system installation by extracting `libnspr4`, `libnss3`, and `libasound2t64` into `/tmp/spriteforge-browser-libs`; use its `usr/lib/x86_64-linux-gnu` as `LD_LIBRARY_PATH` in this environment.
- Validation: 20 unit tests, production build, all 10 existing browser tests and new complete-character/play/export browser test pass. Added original seven-clip ForgeKnight demo fixture.
- Unreal character milestone: extended the existing importer/actor with a character data asset, preflighted multi-state import, Blueprint state/one-shot APIs, deterministic playback stepping and a ready-to-drop camera/WASD example.
- Actual Unreal Engine 5.8 BuildPlugin passed Editor and Win64 runtime packaging. The generated seven-state ForgeKnight imported into `/Game/SpriteForge/Characters/ForgeKnight/`. `tests/unreal/verify_character.py` executed successfully in an isolated Editor project with 0 errors/warnings: every state, frame timing, loop wrap, attack return, held death, heading-relative directions and example construction. These are `-nullrhi` runtime tests, not a visual PIE gameplay review.
- Factory/population milestone: core/custom slots, hierarchy + rest-transform signatures, strict shared-bind-pose equipment, bone sockets/XYZ transforms, bone-only shared animation libraries, SHA-256 source/sidecar references, portable loadouts, persistent per-asset setup and catalog filters.
- Deterministic constrained randomisation uses bounded mixed-radix combinations with locks and duplicate prevention, without constructing Cartesian products. Visual variation cards support rename/favourite/delete/reroll/locks/selection. Sequential bake/export preserves completed characters on cancellation, validates names and presents exact sampled frames plus uncompressed atlas estimates. Population limit is 200; large jobs require workload acknowledgement.
- Added original modular fixtures and a City Guard demo with 60 compatible combinations. The full 13-test browser suite passed, including 50 distinct rendered previews, loadout reload, shared animation library, skinned/socket assembly, selected population bake, duplicate exhaustion, cancellation, and workspace persistence. Unit suite additionally covers 200 unique combinations from a million-combination pool.
- Character states reuse one loaded assembly and sampled union bounds. Atlas-only rendering streams directly into the existing atlas writer instead of retaining per-frame PNGs. Three.js skeleton resources and preview URLs are disposed.
- Final expansion verification: 28 unit tests, 13 browser workflows (including restart/reimport/loadout reload), production build and original + character desktop-CSP smoke passed. Documentation now covers both entry paths, all three demos, metadata/loadout formats, limits and Unreal use, with UI captures.
- GitHub Actions for factory milestone `9634e19` passed web, Windows desktop packaging and Linux desktop packaging. Local Unreal Editor/runtime packaging and actual headless character + original asset importer/runtime tests passed. Visual PIE input is explicitly not claimed.
- Final Unreal precision regression: recompiled the Editor plugin after using a tolerance for JSON-double/Unreal-float front-angle comparison; a synthetic seven-state package with a 12.34° front convention imported successfully. New-asset lookup warnings are expected during first import; the commandlet returned success.
