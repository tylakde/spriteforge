...
- **28 unit tests passed:** original math/framing/recipes/styles, animation classification (including abilities/custom names), loop defaults, mapping validation, character metadata/package output, runtime timing/directions, attachment/loadout serialization, skeleton signatures/compatibility, filename slots, GLTF sidecar fingerprints, randomisation constraints/locks/exhaustion, duplicate prevention and population estimates. The generator test creates 200 distinct combinations from a million-combination pool without enumerating it.
- **13 browser tests passed:** all 10 original workflows plus complete character → baked playable preview → package export; real skinned/socket assembly with a bone-only animation library, loadout reload both in-session and after restart, variation previews/selected bake; and 50 distinct City Guard previews, workload acknowledgement, 200-count cap, cancellation and workspace persistence.
- **Production build and desktop-CSP smoke passed:** existing static bake/export and complete Knight build, actual-sprite movement, and Unreal package export in the production bundle. Format checks pass.
- **Unreal Engine 5.8, MSVC 14.44:** plugin Editor/Win64 Development/Shipping packaging passed. The seven-state generated Knight was actually imported into a separate temporary UE project. `verify_character.py` executed successfully with 0 errors/warnings, covering all states, frame advancement, looping, attack return, held death, camera-relative selection and demo actor construction. Original Runestone/Sentinel importer/runtime regressions also passed. This is headless `-nullrhi` validation, not a visual PIE/input playthrough.
- **GitHub Actions at factory milestone `9634e19`:** web verification, Windows desktop packaging and Linux desktop packaging all passed. This is separate from local browser/Unreal checks.

Reproduce browser tests with `npm test`, `npm run test:e2e`, and `npm run test:production`. `npm run fixtures` regenerates all original procedural fixtures, including Knight and the modular demo pool. No external artwork is required. On this local Ubuntu 26.04 environment, Chromium's missing libraries were extracted under `/tmp/spriteforge-browser-libs`; prefix browser commands with `LD_LIBRARY_PATH=/tmp/spriteforge-browser-libs/usr/lib/x86_64-linux-gnu`. Standard CI uses Playwright's normal dependency installation.

For Unreal, export the ForgeKnight demo at 8 directions, 128 px and 4 FPS, install/build the plugin in an isolated project with PythonScriptPlugin enabled, then run `scripts/verify-character-unreal.ps1 -Project <project.uproject> -CharacterMetadata <ForgeKnight.character.json>`. Existing standalone importer validation remains in `scripts/verify-unreal.ps1`.

Reviewable UI captures: `docs/character-forge.png`, `docs/character-play-test.png`, `docs/population-factory.png`. The three demo workflows are documented in `CHARACTER_FORGE.md` and `CHARACTER_FACTORY.md`.

A final precision regression also compiled and imported a synthetic seven-state package with a **12.34° front convention**. Character-level JSON doubles are compared to Unreal float fields with tolerance. Initial asset-lookup warnings during first import are expected; the commandlet returned success with zero errors.
