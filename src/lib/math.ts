...
export function directionAngles(count: number, front = 0): number[] {
  if (![1, 4, 8, 16, 32].includes(count))
    throw new Error("Unsupported direction count.");
  return Array.from({ length: count }, (_, i) =>
    normaliseAngle(front + (i * 360) / count),
  );
}
export function sampleTimes(duration: number, fps: number): number[] {
  if (
    ...
    throw new Error("Animation duration or FPS is invalid.");
  const count = Math.ceil(duration * fps);
  if (count > 4096)
    throw new Error(
      "Animation exceeds 4096 samples. Reduce FPS or split the clip.",
    );
...
export function safeName(name: string): string {
  const clean =
    name
      ...
      .replace(/^_+|_+$/g, "")
      .slice(0, 80) || "Asset";
  return /^(con|prn|aux|nul|com[0-9]|lpt[0-9])$/i.test(clean)
    ? `_${clean}`
    : clean;
}
export function frameName(
  ...
  const degrees = Number(normaliseAngle(angle).toFixed(3))
    .toString()
    .padStart(3, "0")
    .replace(".", "p");
...
export function atlasLayout(
  ...
  const columns = layout === "strip" ? count : Math.ceil(Math.sqrt(count));
  const rows = Math.ceil(count / columns);
  const round = (v: number) => (pot ? 2 ** Math.ceil(Math.log2(v)) : v);
  const width = round(columns * stride),
    height = round(rows * stride);
  if (width > 8192 || height > 8192 || width * height > 33554432)
    throw new Error(
      "Atlas is too large (limit 8192px / 32 megapixels). Reduce cell size, directions or FPS, or use grid layout.",
    );
  return {
    width,
    height,
