      ...
      for (let dy = -width; dy <= width; dy++)
        for (let dx = -width; dx <= width; dx++) {
          ...
          if (xx >= 0 && xx < size && yy >= 0 && yy < size)
            edge = Math.max(edge, alpha[yy * size + xx]);
        }
      const a = alpha[i] / 255,
        b = (edge / 255) * (1 - a),
        out = a + b;
...
export async function renderSequence(
  ...
  try {
    ...
    if (clipIndex !== null && !clip)
      throw new Error("Selected animation is missing.");
    ...
    if (clip) {
      ...
      for (let i = 0; !options?.bounds && i < times.length; i++) {
        ...
        if (i % 8 === 0) {
          onProgress((i / times.length) * 0.1, "Measuring animation bounds");
          await yieldToUI();
        }
      }
    ...
    for (const frame of metadata.frames) {
      ...
      const camera = renderCamera(framing, recipe, frame.directionDegrees);
      renderer.setRenderTarget(target);
      ...
      if (recipe.background === "solid") {
        ...
        ctx.fillStyle = recipe.backgroundColor;
...
export function disposeGeneration(generation: Generation | null) {
  generation?.thumbnails.forEach(URL.revokeObjectURL);
}
