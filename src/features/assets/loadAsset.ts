...
export function sourcesFromFiles(files:File[]):AssetSource[]{
  return files.filter(f=>/\.(glb|gltf)$/i.test(f.name)).map(file=>({id:crypto.randomUUID(),name:file.name,file,files}));
}
export function disposeObject(root:Group){
 const geometries=new Set(),materials=new Set(),textures=new Set<Texture>();
 root.traverse(o=>{if(o instanceof Mesh){if(!geometries.has(o.geometry)){o.geometry.dispose();geometries.add(o.geometry);}for(const m of Array.isArray(o.material)?o.material:[o.material]) if(!materials.has(m)){materials.add(m);for(const value of Object.values(m)) if(value instanceof Texture)textures.add(value);m.dispose();}}});
 for(const t of textures){t.dispose();const data=t.source?.data;if(typeof ImageBitmap!=='undefined'&&data instanceof ImageBitmap)data.close();}
}
export async function loadAsset(source:AssetSource):Promise<LoadedAsset>{
 const urls:string[]=[]; const missing:string[]=[];const manager=new LoadingManager();
 const files=new Map<string,File>();
 for(const f of source.files){const path=f.webkitRelativePath||f.name;files.set(path.replace(/\\/g,'/'),f);}
 const sourcePath=source.file.webkitRelativePath||source.file.name; const dir=sourcePath.slice(0,sourcePath.lastIndexOf('/')+1);
 manager.setURLModifier(url=>{
   if(url.startsWith('data:')||url.startsWith('blob:'))return url;
   const decoded=decodeURIComponent(url).replace(/^\.\//,'');
   const normalised=new URL(decoded,'https://local/'+dir).pathname.slice(1);
   let file=files.get(normalised)||files.get(decoded);
   if(!file){const matches=source.files.filter(f=>f.name===decoded.split('/').pop());if(matches.length===1)file=matches[0];}
   if(!file){missing.push(decoded);throw new Error(`Missing resource: ${decoded}. Import the asset folder with its .bin and texture files.`);}
 ...
 try {
  ...
  if(bounds.isEmpty()||![...bounds.min.toArray(),...bounds.max.toArray()].every(Number.isFinite))throw new Error('Asset has no valid visible geometry.');
  const center=bounds.getCenter(new Vector3());gltf.scene.position.add(new Vector3(-center.x,-bounds.min.y,-center.z));root.updateMatrixWorld(true);
  bounds=new Box3().setFromObject(root,true);const dimensions=bounds.getSize(new Vector3()).toArray() as [number,number,number];
