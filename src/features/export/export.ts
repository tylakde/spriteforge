...
export async function exportRecipe(text:string,name:string){
 if(isTauri()){const path=await save({defaultPath:`${safeName(name)}.recipe.json`,filters:[{name:'Recipe JSON',extensions:['json']}]});if(path)await writeFile(path,new TextEncoder().encode(text));}
 else download(new Blob([text],{type:'application/json'}),`${safeName(name)}.recipe.json`);
}
export async function chooseOutputDirectory(){return isTauri()?await open({directory:true,multiple:false,title:'Choose SpriteForge output folder'}):null;}
export async function exportGeneration(generation:Generation,mode:ExportMode,directory?:string|null){
 const files:Record<string,Uint8Array>={};const {metadata}=generation;
 if(mode!=='frames'){files[metadata.atlas.file]=new Uint8Array(await generation.atlas.arrayBuffer());files[`${metadata.asset}.json`]=strToU8(JSON.stringify(metadata,null,2));}
 if(mode!=='atlas')for(let i=0;i<generation.frames.length;i++)files[`frames/${metadata.frames[i].file}`]=new Uint8Array(await generation.frames[i].arrayBuffer());
 if(isTauri()){
  const output=directory??await chooseOutputDirectory();if(!output)return null;
  // Rust creates a fresh directory atomically and only writes safe relative filenames into it.
  return await invoke<string>('export_files',{directory:output,name:metadata.asset,files:Object.entries(files).map(([path,data])=>({path,data:Array.from(data)}))});
 }
 const data=await new Promise<Uint8Array>((resolve,reject)=>zip(files,{level:0},(error,result)=>error?reject(error):resolve(result)));
 const name=`${metadata.asset}_${mode}.zip`;download(new Blob([new Uint8Array(data)],{type:'application/zip'}),name);return name;
}
