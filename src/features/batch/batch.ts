...
export async function runBatch(
  ...
  for (const source of sources) {
    ...
    let generation = null;
    try {
      generation = await renderSequence(
        source,
        recipe,
        animated ? 0 : null,
        (progress, detail) =>
      ...
      if (signal.aborted) break;
      const path = await exportGeneration(generation, mode, directory);
      onUpdate({
        id: source.id,
        name: source.name,
        status: "done",
        detail: path || "Export cancelled",
        progress: 1,
      });
    } catch (e) {
      onUpdate({
        ...
        name: source.name,
        status: "error",
        detail: e instanceof Error ? e.message : "Batch item failed.",
        progress: 0,
      });
    } finally {
