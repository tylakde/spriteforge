...
export function calculateFraming(bounds: Box3, recipe: RenderRecipe) {
  ...
  const elevation = MathUtils.degToRad(recipe.cameraElevation);
  const height = bounds.max.y - bounds.min.y;
  const vertical =
    height * Math.abs(Math.cos(elevation)) +
  ...
  if (recipe.anchor === "ground") {
    ...
    const low =
      bounds.min.y * Math.cos(elevation) -
      radius * Math.abs(Math.sin(elevation));
    const high =
      bounds.max.y * Math.cos(elevation) +
      radius * Math.abs(Math.sin(elevation));
  ...
  const target = new Vector3(
    0,
    targetY / Math.max(Math.cos(elevation), 0.017),
    0,
  );
  const sphere = bounds.getBoundingSphere({
    center: new Vector3(),
    radius: 0,
  } as import("three").Sphere).radius;
  // Perspective uses a conservative depth allowance for every yaw to avoid close-side clipping.
  const distance =
    recipe.projection === "perspective"
      ? span / (2 * Math.tan(MathUtils.degToRad(recipe.perspectiveFov / 2))) +
        sphere * 2
      : span * 3 + sphere * 2;
  if (recipe.projection === "perspective")
    span =
      2 * distance * Math.tan(MathUtils.degToRad(recipe.perspectiveFov / 2));
  return { target, span, distance, pivotY: 0.5 + targetY / span };
}
export function renderCamera(
  framing: ReturnType<typeof calculateFraming>,
  recipe: RenderRecipe,
  angle: number,
) {
  const { span, distance, target } = framing;
  const near = Math.max(0.0001, distance / 10000),
    far = distance * 10 + span * 10;
  const camera =
    recipe.projection === "orthographic"
      ...
      : new PerspectiveCamera(recipe.perspectiveFov, 1, near, far);
  const yaw = MathUtils.degToRad(angle),
    elevation = MathUtils.degToRad(recipe.cameraElevation);
  camera.position
    .copy(target)
    .add(
      new Vector3(
