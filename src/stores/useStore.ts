...
interface State { recipe:RenderRecipe; presets:RenderRecipe[]; grid:boolean; previewBackground:'checker'|'solid'; outputMode:'all'|'atlas'|'frames';
 update:(patch:Partial<RenderRecipe>)=>void; setRecipe:(recipe:RenderRecipe)=>void; savePreset:(name:string)=>void; deletePreset:(name:string)=>void; reset:()=>void; preferences:(patch:Partial<Pick<State,'grid'|'previewBackground'|'outputMode'>>)=>void; }
